...
beforeAll(async () => {
  ...
  pool = await ensureTestPool("E01-S03", "moneo_e01_tenancy", ["background_job_attempts", "job_dispatch_index", "outbox_events", "background_job_results", "background_jobs", "ai_dispatch_permits", "ai_exclusions", "ai_policies", "command_operations", "accounts", "workspace_members", "workspaces", "users", "app_sessions"]);
...
describe("e01-s03 tenant ownership", () => {
  ...
  it("migrations 006 then 005 then 004 then 003 then 002 roll back and re-apply on the suite database", async () => {
    const { readFileSync } = await import("node:fs");
    // Newest first: 006's attempts reference jobs, 005's jobs reference
    // workspaces/command_operations, 004's exclusions reference accounts,
    // and 002's rollback drops the accounts table that carries 003's
    // version column, so newest-first while recorded — otherwise re-migrate
    // never restores the dependent objects.
    for (const file of ["006_job_recovery.rollback.sql", "005_jobs.rollback.sql", "004_ai_policy.rollback.sql", "003_commands.rollback.sql", "002_tenancy.rollback.sql"]) {
    ...
    const gone = await pool.query("SELECT count(*)::int AS n FROM pg_tables WHERE tablename IN ('users', 'workspaces', 'workspace_members', 'accounts', 'command_operations', 'ai_policies', 'ai_exclusions', 'ai_dispatch_permits', 'background_jobs', 'background_job_results', 'outbox_events', 'job_dispatch_index', 'background_job_attempts')");
    expect((gone.rows[0] as { n: number }).n).toBe(0);
    // Self-healing: the idempotent migrator restores the full shape,
    // including 003's version column/journal, the 004 policy tables, the
    // 005 job/outbox/dispatch tables and the 006 attempt history.
    await migrate(pool, "apps/web/migrations");
    const back = await pool.query("SELECT count(*)::int AS n FROM pg_tables WHERE tablename IN ('users', 'workspaces', 'workspace_members', 'accounts', 'command_operations', 'ai_policies', 'ai_exclusions', 'ai_dispatch_permits', 'background_jobs', 'background_job_results', 'outbox_events', 'job_dispatch_index', 'background_job_attempts')");
    expect((back.rows[0] as { n: number }).n).toBe(13);
