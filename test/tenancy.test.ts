...
beforeAll(async () => {
  ...
  pool = await ensureTestPool("E01-S03", "moneo_e01_tenancy_v13", ["recurring_overrides", "transaction_tags", "audit_events", "tags", "categories", "workspace_data_revision", "calculation_versions", "fx_valuation", "fx_rates_ecb", "fx_rates_manual", "manual_transactions", "balance_snapshots", "balance_audit", "mapping_provider_usage", "mapping_provider_reservations", "mapping_proposals", "mapping_profiles", "review_decisions", "source_links", "transactions", "import_commit_batches", "parsed_observations", "source_objects", "imports", "data_sources", "background_job_attempts", "job_dispatch_index", "outbox_events", "background_job_results", "background_jobs", "ai_dispatch_permits", "ai_exclusions", "ai_policies", "command_operations", "accounts", "workspace_members", "workspaces", "users", "app_sessions", "artifact_build_attempts", "artifact_versions", "artifacts", "artifact_runtime_grants", "artifact_sdk_access_events", "artifact_state", "artifact_state_snapshots", "artifact_state_migrations", "artifact_ai_proposals", "projection_settings", "financial_assumptions", "goals", "goal_allocations"]);
...
describe("e01-s03 tenant ownership", () => {
  ...
  it("migrations 034 down to 002 roll back and re-apply on the suite database", async () => {
      ...
      expect((back.rows[0] as { n: number }).n).toBe(59);
