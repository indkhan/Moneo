...
beforeAll(async () => {
  ...
  pool = await ensureTestPool("E01-S03", "moneo_e01_tenancy", ["parsed_observations", "source_objects", "imports", "data_sources", "background_job_attempts", "job_dispatch_index", "outbox_events", "background_job_results", "background_jobs", "ai_dispatch_permits", "ai_exclusions", "ai_policies", "command_operations", "accounts", "workspace_members", "workspaces", "users", "app_sessions"]);
...
describe("e01-s03 tenant ownership", () => {
  ...
  it("migrations 007 then 006 then 005 then 004 then 003 then 002 roll back and re-apply on the suite database", async () => {
    const { readFileSync } = await import("node:fs");
    // Newest first: 007's staging tables reference workspaces/imports, 006's
    // attempts reference jobs, 005's jobs reference workspaces/operations,
    // 004's exclusions reference accounts, and 002's rollback drops the
    // accounts table that carries 003's version column — newest-first while
    // recorded, otherwise re-migrate never restores the dependents.
    for (const file of ["007_uploads.rollback.sql", "006_job_recovery.rollback.sql", "005_jobs.rollback.sql", "004_ai_policy.rollback.sql", "003_commands.rollback.sql", "002_tenancy.rollback.sql"]) {
    ...
    expect((back.rows[0] as { n: number }).n).toBe(17);
