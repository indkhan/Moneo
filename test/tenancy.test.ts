...
describe("e01-s03 tenant ownership", () => {
  ...
  it("app role is least-privilege and RLS is forced", async () => {
    ...
    const forced = await pool.query("SELECT relname, relforcerowsecurity AS forced FROM pg_class WHERE relname IN ('workspaces', 'workspace_members', 'accounts', 'artifacts', 'artifact_versions', 'artifact_build_attempts', 'artifact_runtime_grants', 'artifact_sdk_access_events', 'artifact_state', 'artifact_state_snapshots', 'artifact_state_migrations', 'artifact_ai_proposals', 'projection_settings', 'financial_assumptions', 'goals', 'goal_allocations', 'projection_runs', 'projection_points', 'projection_events', 'deep_analysis_runs', 'deep_analysis_steps', 'deep_analysis_findings')");
    expect(forced.rows).toHaveLength(22);
  ...
  it("migrations 037 down to 002 roll back and re-apply on the suite database", async () => {
      ...
      expect((back.rows[0] as { n: number }).n).toBe(65);
