...
**R1 deployment/identity override (2026-09-17):** Founder decision superseding vendor-specific R1 references below: R1 uses a self-hosted, pinned Keycloak container as its OIDC/WebAuthn identity provider and Docker-hosted application/worker containers instead of Auth0, Vercel, Render and AWS federation. Security contracts remain unchanged: Authorization Code + PKCE, server-side application sessions, strong-factor enrollment, local session revocation, tenant isolation, non-root/read-only services, least-privilege per-service secrets, durable storage outside container filesystems and no long-lived cloud credentials. Docker Compose secrets are the initial service-secret boundary. Production exposure still requires TLS, persistent PostgreSQL-backed Keycloak, backups/restore proof, patched-image rollout and an explicit hosting/network decision. Keycloak `start-dev` is proof-only and never production configuration. Where later text names superseded vendors, apply these R1 replacements without weakening the stated invariant.
