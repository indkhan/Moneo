...
Failure lifecycle: Concurrent same-key races converge via UNIQUE + re-read (one SUCCEEDED row); crash mid-tx rolls back claim+effect together (atomic executor); ambiguous transport outcome → client reconciles via GET/replay, never blind re-execution with a new key (documented); rollback file drops the table + version column (synthetic data only).
UI/accessibility: Not applicable — no browser UI (S06 owns the shell).
