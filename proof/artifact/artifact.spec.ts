...
declare global { interface Window { proof: { state(): { slider: number }; activate(source: any, migrate: (value: any) => any): boolean; hostile(js: string): void } } }
...
test("sanitizes HTML and rejects resource-loading CSS", async ({ page }) => {
  ...
  await page.evaluate(() => window.proof.activate({
