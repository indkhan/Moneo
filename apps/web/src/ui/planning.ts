...
export async function handlePlanningRoutes(
  ...
  if (scenarioDetailMatch && method === "GET") {
    ...
    try {
      ...
      const aggNote = cmp.aggregated === "weekly" ? `<p>Weekly samples (every 7th day plus horizon end) over this long horizon; values are exact.</p>` : "";
      const truncNote = cmp.truncated ? `<p>Results truncated at the display cap; refine the horizon for full detail.</p>` : "";
      html(
        res,
        200,
        page({
