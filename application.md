...
- Built-in categorization matches counterparties by contained merchant phrase against a dictionary seeded from real Commerzbank and Revolut exports; Revolut pocket/top-up movements auto-label as `transfer.internal`, which spending summaries exclude.
...
- `docs/moneo-roadmap.html` defines the next web-only ledger slice. After privacy containment, one exact second-format statement can be previewed, saved to a labelled local profile, atomically replaced rather than merged, and reported with reconciled per-currency balances and flows at narrow and wide browser widths; native persistence and import remain out of scope.
